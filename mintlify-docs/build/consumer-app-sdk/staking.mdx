...
<Warning>
  Staking requires `staking.contract` configuration in the SDK. See [Configuration](/build/consumer-app-sdk/configuration#staking-configuration) for setup.
</Warning>
...
5. **Keep staking contract config chain-aware** (different addresses per network)
